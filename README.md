...
Include the attribute server into your plutus project by either using this project as a template or by copying the files under `app` and `src` into your project. Note that this project uses IOG's plutus-starter as its template.
...
```
cabal build attribute-server  
```

3. Run the web server for script address generation

```
 ./start-attribute-server.sh
```
