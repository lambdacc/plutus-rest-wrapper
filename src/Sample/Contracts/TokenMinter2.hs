...
module Sample.Contracts.TokenMinter2 (
  module Sample.Contracts.TokenMinter2,
...
toMintingPolicyParams :: MintParams -> PaymentPubKeyHash -> Integer -> MintingPolicyParam
toMintingPolicyParams rp p rand =
  MintingPolicyParam
    { issuer = p
    , tName = TokenName { unTokenName = stringToBuiltinByteString (tn rp)}
    , randomiser = rand
    }
...
mintToken rp = do
  ...
  time <- currentTime
  let mp = toMintingPolicyParams rp ownPkh (getPOSIXTime time)
